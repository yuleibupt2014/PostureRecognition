...
namespace Chronos_As_Read
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            Problem test3 = Problem.Read("test3.txt");
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (list.Count >= 180)
            {
                ...
                for (int j = 0; j < 180; j++)
                {
                    ...
                    x[j].Value = Convert.ToDouble(Form1.list[j]);      //X【j】.value代表第几个特征值
                }
                double v;
        ...
        public void training_Click(object sender, EventArgs e)
        {
           // Double zhengquelv = 0d;
            train_click_flag = 1;
            Problem train = Problem.Read("演示.txt");       //train里面包含vx.count  vx  vy   train._X=vx(即训练输入数据)   train._Y=vy（训练类标签）
