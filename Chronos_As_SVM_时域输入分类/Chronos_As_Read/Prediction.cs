    ...
    public class Prediction
    {
            ...
            bool predict_probability)
        {
            ...
            if (predict_probability)
            {
                ...
                for (int i = 0; i < 200; i++)   //执行一次，这里是有多少个训练样例便执行多少次循环
                {
                    double target = problem.Y[i];
                    Node[] x = problem.X[i];                    ///在这里改！！！！把输入改成Node 【】 X的形势！！！！！！！！！！！！！！！！！！！
                    double v;
        ...
        public static double  Pred(Model model, Node[] x)         //这里是自己的设置
        {
